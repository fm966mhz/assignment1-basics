...
def run_linear(
    ...
    Args:
        ...
        weights (Float[Tensor, "dout d_in"]): The linear weights to use
...
def run_swiglu(
    ...
    swiglu.load_state_dict(
        {
            "in_projection_layer_1.weight": w1_weight,
            "out_projection_layer_2.weight": w2_weight,
            "in_projection_layer_3.weight": w3_weight,
        }
    )
...
def run_transformer_block(
    ...
    transformer_block = TransfomerBlock(
        d_model=d_model, num_heads=num_heads, d_ff=d_ff, rope=rope
    )
    transformer_block.load_state_dict(
        correct_transformer_block_weights_dict(input_dict=weights, path_prefix="")
    )
    return transformer_block(
        in_features=in_features, token_positions=torch.arange(in_features.shape[-2])
    )
...
def run_train_bpe(
    ...
    **kwargs,
    ...
    return tokenizer.train(str(input_path))
